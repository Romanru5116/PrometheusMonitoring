#Grafana 
#cкритерии что графана может быть разаернутаna из Git-репозитория на чистый сервер,
1. конфигурация должна отвечать принципам Infrastructure-as-Code и Observability-as-Code:
  2.Все необходимые файлы настроек, дашбордов и источников данных должны храниться в декларативном виде, чтобы новая инсталляция автоматически воссоздала точное состояние системы:
     1.Декларативное описание (Provisioning)Источники данных (Data Sources):
       1.Описаны в YAML-файлах в папке provisioning/datasources/ с указанием типов, URL и параметров аутентификации.Дашборды (Dashboards):
         2.Экспортированы в виде JSON-файлов и структурированы по папкам через YAML-манифесты в provisioning/dashboards/.
         6. Настройки окружения: Основной конфигурационный файл grafana.ini (или его аналог через переменные среды) вынесен в репозиторий.
         7.Управление доступом: Пользователи, команды и роли (RBAC) также задекларированы, если поддерживаются версией и плагинами.
         8.Автоматизация и инструменты развертыванияСкрипт или CI/CD Pipeline: Наличие Ansible, Docker Compose, Helm-чарта или Makefile, которые скачивают репозиторий на чистый сервер.
         9.Идемпотентность: Процесс установки можно запускать повторно без риска сломать систему или задублировать сущности.
         10.Версионирование плагинов: Список необходимых плагинов (gf_security / переменные окружения для установки GF_INSTALL_PLUGINS) зафиксирован в конфигурации сборки.
          11.Управление секретами и доступВнешние секреты: Пароли, токены доступа к источникам данных и ключи шифрования не зашиты в открытом виде в Git, а передаются через секреты ОС, .env файлы или менеджеры секретов (Vault).Сетевая доступность:
           12. Сомнительно: Новый сервер имеет открытые сетевые маршруты до самих источников данных (Prometheus, Loki и т.д.) и доступ к приватному/публичному Git-репозиторию.

структура обмена файлов
...
# какие есть модели мониторинга
Методология USE (Utilization, Saturation, Errors) помогает анализировать состояние ресурсов сервера. насколько загружен ремурс Очередь  Объекты мониторинга делятся на уровни: 
методология: RED RAteErrorsDuration - для сервисов 
синтетичекий мониторинг - как снаружи видит сайт
...
одход к хранению графана ка код и созданию конвейера https://grafana.com/docs/grafana-cloud/learn-and-build/as-code/
