...
продолжаем переносить конфиги графаны в git (инфраструктура как код
под нее сделана структура директорий
![](https://github.com/Romanru5116/PrometheusMonitoring/blob/9f755e4764dfe29f2e52610db38a1ca30296de1d/PIC/ScreenshotGrafanaNewDir%20from%202026-07-31%2003-52-52.png)

Внимание! гипотеза просто перенести всю БД Графаны в Git - неоправдалось - небезопасно- н занесем все эти директории в исключения (сдедано)
соз

вот подсказка
Развернуть дашборды «вместе с БД» (то есть импортировав их один раз в grafana.db и забыв про Git) — это рабочий, но небезопасный подход для GitOps. Если контейнер Grafana удалится или повредится grafana.db, вы потеряете все свои дашборды.Правильный путь для Docker Compose — использовать официальный механизм Grafana Provisioning. Вы храните дашборды в Git как обычные JSON-файлы, а Grafana сама автоматически считывает их и загружает в свою БД при каждом запуске.

## статус 230726
добавлен график на панель -ввод ввывод дисков сколько времени занимал - скорее даже среднее значение
![ввод-вывод](https://github.com/Romanru5116/PrometheusMonitoring/blob/409f3b58813e647c496611584050fc096678521b/PIC/ScreenshotGrafana_IO%20chg_rate%20from%202026-07-23%2002-55-18.png)
...
Задачи 1,4:
итого:
*использован современный подход развертывания  через docker compose
 * развернута Графана
 * развернут сам сервер прометеус
 * развернут Алертменеджер 
* развернут node exporter
* развернут procces exporter
* все это проинтегрировано
* выведены дашборды
![](https://github.com/Romanru5116/PrometheusMonitoring/blob/main/PIC%2FScreenshotDasGarafana%20from%202026-07-16%2011-24-26.png)
 * конфиг всего этого вышеописанного перенесен в гит( концепт инфраструктура как код)- применительно к данной инсталляции- развертывание в любую директорию или ПК прямо из git

* пока не проводились испытания развертывания с "0"- можно считать это как CD 

Задача2: развернут кубер в виде миникьюб
Задача 3: deployment написан, надо тестировать развертывание
